...
@section('title'){{'Sales Create'}}@endsection
@section('css')
<style>
    INPUT:not(:autofill),
    SELECT:not(:autofill),
...
<div class="container-fluid mt-3 position-relative">
    <div class="container">
        <div class="row">
            <p style="font-size: 15px; margin-left:35px;"><span style="color:#4EA6D7;">Sales</span><span style="color: #B8B8B8; margin: 0 10px;">/</span><span style="color: #B8B8B8;"> New Sale</span></p>
        </div>
        <div class="row">
            <div class="col-5"></div>
            <div class="col-3">
            ...
            <div class="row">
                <div class="col-sm-3">
                    <div class="mb-3 row ">
                        ...
                        <div class="col-sm-8">
                            <div class="d-flex">
                                <select class="form-select" name="customer" style="font-size: 14px;font-weight: 400;color: #555; border-color:#ffffff" required id="selection" size="1">
                                    <option value=""></option>
                                </select>
                    ...
                    <div class="mb-3 row">
                        ...
                        <div class="col-sm-8">
                            <input type="date" class="form-control" data-date-format="DD/MM/YYYY" name="sale_date" style="border: none;font-size: 14px;font-weight: 400;color: #B5B2B2;" id="date" value="{{old('sale_date')}}" required>
                        </div>
                    </div>
                    <div class="mb-3 row">
                    ...
                    <div class="mb-3 row">
                        ...
                        <div class="col-sm-8">
                            <button type="submit" class="btn btn-primary" style="background-color: #1F5A92; text-transform: none;  border-radius: 0; width: 160px;height: 43px;"> <span style="font-size: 14px; font-weight:400; color:#ffffff"> Save</span></button>
                        </div>
                    </div>
                </div>
                ...
                success: function(response) {
                    ...
                    if (rowCount == 0) {
                        ...
                        var table = '<tr id="row' + index + '" style="background-color: #FFFFFF; border-left-width:1px;border-right-width:1px; " class="row-append">\
                                            ...
                                            <td class="import-text-color-body">\
                                                <input type="text" required value="{{old("sale_details[' + index + '][qty]")}}" name="sale_details[' + index + '][qty]" style="width: 77px; height:30px; border:1px solid #E0E0E0; border-radius: 0px; background-color:FFFFFF;text-align:center;" class="form-control">\
                                            </td>\
                                            <td class="import-text-color-body">\
                                                <input type="text" required value="{{old("sale_details[' + index + '][unit_price]")}}" name="sale_details[' + index + '][unit_price]" style="width: 77px; height:30px; border:1px solid #E0E0E0; border-radius: 0px; background-color:#FFFFFF; text-align:center;" class="form-control">\
                                            </td>\
                                            <td class="import-text-color-body">\
                        ...
                        if (!isFound) {
                            ...
                            var table = '<tr id="row' + index + '" style="background-color: #FFFFFF; border-left-width:1px;border-right-width:1px; " class="row-append">\
                                            ...
                                            <td class="import-text-color-body">\
                                                <input type="text"required value="{{old("qty")}}" name="sale_details[' + index + '][qty]" style="width: 77px; height:30px; border:1px solid #E0E0E0; border-radius: 0px; background-color:FFFFFF;text-align:center;" class="form-control">\
                                            </td>\
                                            <td class="import-text-color-body">\
                                                <input type="text"required value="{{old("unit_price")}}" name="sale_details[' + index + '][unit_price]" style="width: 77px; height:30px; border:1px solid #E0E0E0; border-radius: 0px; background-color:#FFFFFF; text-align:center;" class="form-control">\
                                            </td>\
                                            <td class="import-text-color-body">\
                                                <button type="button" class="btn btn-primary remove_row" id="' + index + '" style="background-color: #E85757; text-transform: none;  border-radius: 0; width: 90px;height:30px; padding:0px;"><span>Remove</span></button>\
                                            </td>\
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.status == 400) {
                            ...
                            $('#phone').val('');
                            $('#address').val('');
                            $('.status_success').text(response.message);
            ...
            $.ajax({
                ...
                success: function(response) {
                    $.each(response.customers, function(key, item) {
                        var select = '<option value="' + item.name + ' ">' + item.name + '</option>'
                        $('#selection').append(select);
                    });
                }
            });
